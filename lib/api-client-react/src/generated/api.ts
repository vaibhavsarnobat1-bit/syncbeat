 ...
 */
export const getLoginAnonymousUrl = () => {
  return `/api/auth/anonymous`;
};
...
export const loginAnonymous = async (
  ...
  return customFetch<UserSession>(getLoginAnonymousUrl(), {
    ...options,
    method: "POST",
    headers: { "Content-Type": "application/json", ...options?.headers },
    body: JSON.stringify(anonymousLoginRequest),
  });
};

export const getLoginAnonymousMutationOptions = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof loginAnonymous>>,
    TError,
    { data: BodyType<AnonymousLoginRequest> },
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationOptions<
  ...
  TContext
> => {
  const mutationKey = ["loginAnonymous"];
  ...
  > = (props) => {
    ...
    return loginAnonymous(data, requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type LoginAnonymousMutationResult = NonNullable<
  Awaited<ReturnType<typeof loginAnonymous>>
>;
export type LoginAnonymousMutationBody = BodyType<AnonymousLoginRequest>;
export type LoginAnonymousMutationError = ErrorType<unknown>;
...
>(options?: {
  ...
  TContext
> => {
  return useMutation(getLoginAnonymousMutationOptions(options));
};
 ...
 */
export const getGetRoomUrl = (inviteCode: string) => {
  return `/api/rooms/${inviteCode}`;
};
...
export const getRoom = async (
  ...
  return customFetch<Room>(getGetRoomUrl(inviteCode), {
    ...options,
    method: "GET",
  });
};

export const getGetRoomQueryKey = (inviteCode: string) => {
  return [`/api/rooms/${inviteCode}`] as const;
};
...
>(
  ...
  const queryKey = queryOptions?.queryKey ?? getGetRoomQueryKey(inviteCode);

  const queryFn: QueryFunction<Awaited<ReturnType<typeof getRoom>>> = ({
    signal,
  }) => getRoom(inviteCode, { signal, ...requestOptions });
...
>(
  ...
  const queryOptions = getGetRoomQueryOptions(inviteCode, options);
